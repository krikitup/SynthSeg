...
for vol_num in range(1, 353):
    ...
    for type_num in range(1, 5):
        # Load the volume
        volume_filename = os.path.join(volume_directory, f'volume_{vol_num}_type{type_num}.nii.gz')
        ...
        fin_data.append(resized_vol_data)
    # Combine the resized volumes into a 4D volume
    combined_vol_data = np.stack(fin_data, axis=-1)  # Stack along the last dimension
    # Save the combined 4D volume  
    
    fin_vol = nib.Nifti1Image(combined_vol_data, vol_img.affine)
    ...
    # nib.save(final_mask_img, os.path.join(save_mask_dir, f'volume_{vol_num_str}_mask.nii.gz'))
